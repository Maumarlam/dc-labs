...
func logout(c *gin.Context){
  isThere := authentication(c)   //LE PASO TAMBIEN LA C????    ASIGNO CON = o con :=
  var index int
  var userID string
  inList := false
...
func uploadImage(c *gin.Context){
  ...
  if(inList == true){
    ...
    timeATM := time.Now()
    timeATM = timeATM.Format("2006-01-02 15:04:05")
    upImage := filepath.Base(file.Filename)   //Get the name of the file location

    newFile := userToken + "_" + timeATM +"_"+  upImage
    if err:= c.SaveUploadFile(file, newFile); err!=nil{
...
func authentication(c *gin.Context)(string, error){
...
func login(c *gin.Context){
  ...
  if (inList != true){  //IF the user wasn't on the list we need to register it
    ...
    c.JSON(http.StatusOK, gin.H{"Message":"Hi " + userName + ", welcome", "token": token})
  }
}
